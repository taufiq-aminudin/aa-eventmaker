...
class NotificationStore {
  ...
  // Seed realistic notifications and logs for demo accounts
  private seedInitialNotifications() {
    const now = Date.now();

    // Organizer Initial Notifications
    this.addInAppNotification({
      ...
      category: 'ACCOUNT',
      title: 'Selamat Datang di AA Event Maker!',
      message: 'Akun Anda berhasil didaftarkan. Nikmati kemudahan membuat undangan digital, manajemen RSVP, dan buku tamu QR.',
      channel: 'IN_APP',
      isRead: true,
      actionUrl: '/dashboard',
      actionLabel: 'Buka Dasbor',
      createdAt: now - 14 * 24 * 3600 * 1000,
      readAt: now - 14 * 24 * 3600 * 1000,
    });
    ...
    this.addInAppNotification({
      ...
      category: 'PAYMENT',
      title: 'Paket Wedding Professional Aktif',
      message: 'Pembayaran konfirmasi paket Professional telah diverifikasi oleh Administrator. Kuota dan fitur video/musik AI siap digunakan.',
      channel: 'IN_APP',
      isRead: false,
      actionUrl: '/settings',
    ...
    this.addInAppNotification({
      ...
      type: 'RSVP_RECEIVED',
      category: 'RSVP',
      title: 'RSVP Baru: Bpk. Bambang Wijaya',
      message: 'Bpk. Bambang Wijaya mengonfirmasi kehadiran (2 Tamu) pada acara "The Wedding of Andi & Ayu".',
      channel: 'IN_APP',
      isRead: false,
      actionUrl: '/dashboard',
      actionLabel: 'Lihat Daftar RSVP',
    ...
    this.addInAppNotification({
      ...
      category: 'CHECK_IN',
      title: 'Check-In Resepsionis: Ibu Rina Kartika',
      message: 'Ibu Rina Kartika telah berhasil scan E-Pass QR di meja resepsionis (Meja VIP #04).',
      channel: 'IN_APP',
      isRead: false,
      actionUrl: '/dashboard',
      actionLabel: 'Buku Tamu Real-Time',
      createdAt: now - 15 * 60 * 1000,
    });

    // Admin Initial Notifications (Restricted)
    this.addInAppNotification({
      id: 'notif_adm_01',
      ...
      type: 'ADMIN_NEW_USER',
      category: 'ACCOUNT',
      title: 'Pendaftaran Pengguna Baru: Dimas & Ayu',
      message: 'Pengguna baru (dimas.ayu.wedding@gmail.com) mendaftar dengan peran CLIENT dan paket Starter.',
      channel: 'IN_APP',
      isRead: true,
      actionUrl: '/admin/customers',
      actionLabel: 'Lihat Pelanggan',
      createdAt: now - 5 * 24 * 3600 * 1000,
      readAt: now - 5 * 24 * 3600 * 1000,
    });
    ...
    this.addInAppNotification({
      ...
      category: 'PAYMENT',
      title: 'Konfirmasi Pembayaran Baru Memerlukan Verifikasi',
      message: 'Pengguna Taufiq Aminudin mengirimkan bukti transfer Bank Mandiri Rp 299.000 untuk paket Professional (Ref: MANDIRI-AA-99881).',
      channel: 'IN_APP',
      isRead: false,
    ...
    // Initial Email Logs for Admin Auditing
    const sampleEmailLog1: EmailLogRecord = {
      id: 'elog_001',
      idempotencyKey: 'idemp_reg_taufiq',
      userId: 'usr_org_001',
      recipient: 'taufiq.aminudin@gmail.com',
      recipientName: 'Taufiq Aminudin',
      subject: 'Welcome to AA Event Maker — Registration Successful',
      template: 'REGISTRATION_SUCCESS',
      category: 'ACCOUNT',
      status: 'delivered',
      provider: 'AA Mail Gateway / SMTP (Port 587)',
      providerMessageId: 'msg_aa_1715000001_894a',
      attempts: 1,
      maxAttempts: 3,
      lastAttemptAt: now - 14 * 24 * 3600 * 1000,
      sentAt: now - 14 * 24 * 3600 * 1000,
      createdAt: now - 14 * 24 * 3600 * 1000,
    };
    this.emailLogs.set(sampleEmailLog1.id, sampleEmailLog1);

    const sampleEmailLog2: EmailLogRecord = {
      id: 'elog_002',
      idempotencyKey: 'idemp_pay_mandiri99881',
      userId: 'usr_org_001',
      recipient: 'taufiq.aminudin@gmail.com',
      recipientName: 'Taufiq Aminudin',
      subject: 'Payment Confirmed — Your Package Is Active',
      template: 'PAYMENT_APPROVED',
      category: 'PAYMENT',
      status: 'delivered',
      provider: 'AA Mail Gateway / SMTP (Port 587)',
      providerMessageId: 'msg_aa_1715000002_771f',
      attempts: 1,
      maxAttempts: 3,
      lastAttemptAt: now - 2 * 3600 * 1000,
      sentAt: now - 2 * 3600 * 1000,
      createdAt: now - 2 * 3600 * 1000,
    };
    this.emailLogs.set(sampleEmailLog2.id, sampleEmailLog2);

    const sampleEmailLog3: EmailLogRecord = {
      id: 'elog_003',
      idempotencyKey: 'idemp_rsvp_bambang',
      userId: 'usr_org_001',
      recipient: 'taufiq.aminudin@gmail.com',
      recipientName: 'Taufiq Aminudin',
      subject: 'New RSVP Received — The Wedding of Andi & Ayu',
      template: 'RSVP_RECEIVED',
      category: 'RSVP',
      status: 'delivered',
      provider: 'AA Mail Gateway / SMTP (Port 587)',
      providerMessageId: 'msg_aa_1715000003_120c',
      attempts: 1,
      maxAttempts: 3,
      lastAttemptAt: now - 40 * 60 * 1000,
      sentAt: now - 40 * 60 * 1000,
      createdAt: now - 40 * 60 * 1000,
    };
    this.emailLogs.set(sampleEmailLog3.id, sampleEmailLog3);
  }
